...
test('migration runner applies pending migrations transactionally under an advisory lock', async () => {
    ...
    assert.deepEqual(result, { appliedCount: 15, currentVersion: 15 });
    ...
    assert.equal(logs[6].metadata.version, 7);
    assert.equal(logs[6].metadata.name, 'complete_game_history');
    assert.equal(logs[7].metadata.version, 8);
    assert.equal(logs[7].metadata.name, 'game_history_retention');
    assert.equal(logs[8].metadata.version, 9);
    assert.equal(logs[8].metadata.name, 'weekly_attempts');
    assert.equal(logs[9].metadata.version, 10);
    assert.equal(logs[9].metadata.name, 'admin_audit_log');
    assert.equal(logs[10].metadata.version, 11);
    assert.equal(logs[10].metadata.name, 'user_moderation');
    assert.equal(logs[11].metadata.version, 12);
    assert.equal(logs[11].metadata.name, 'user_account_uuid');
    assert.equal(logs[12].metadata.version, 13);
    assert.equal(logs[12].metadata.name, 'admin_operational_controls');
    assert.equal(logs[13].metadata.version, 14);
    assert.equal(logs[13].metadata.name, 'account_activity_days');
    assert.equal(logs[14].metadata.version, 15);
    assert.equal(logs[14].metadata.name, 'password_reset_tokens');
});
...
test('migration runner skips migrations that were already applied with the same checksum', async () => {
    ...
    assert.deepEqual(result, { appliedCount: 0, currentVersion: 15 });
    assert.equal(
