...
test('migration runner applies pending migrations transactionally under an advisory lock', async () => {
    ...
    assert.deepEqual(result, { appliedCount: 1, currentVersion: 1 });
...
test('migration runner skips migrations that were already applied with the same checksum', async () => {
    const [migration] = loadPostgresMigrations({ migrationsDirectoryPath });
    const client = new FakeMigrationClient({
        appliedRows: [{
    ...
    assert.deepEqual(result, { appliedCount: 0, currentVersion: 1 });
    assert.equal(
