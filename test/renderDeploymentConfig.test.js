...
test('Render blueprint documents production web service settings', () => {
    ...
    assert.match(source, /buildCommand:\s*node scripts\/verify-package-lock-integrity\.js && npm ci --include=dev && npm run build/);
...
test('deployment guide includes manual Render checks and secret generation', () => {
    ...
    assert.match(source, /Build Command: node scripts\/verify-package-lock-integrity\.js && npm ci --include=dev && npm run build/);
