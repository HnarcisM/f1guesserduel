...
const state = { activeView: 'dashboard', adminUserId: null, adminAuthorizationMode: 'disabled', userSearch: '', usersRequestSequence: 0, pendingAction: null, selectedUser: null, operationalSettings: null };
...
async function loadUsers() { const requestSequence = ++state.usersRequestSequence; const requestedSearch = state.userSearch; const params = new URLSearchParams({ limit: '50' }); if (requestedSearch) params.set('search', requestedSearch); let payload; try { payload = await api(`/api/admin/users?${params}`); } catch (error) { if (requestSequence !== state.usersRequestSequence || requestedSearch !== state.userSearch) return false; throw error; } if (requestSequence !== state.usersRequestSequence || requestedSearch !== state.userSearch) return false; renderUsers(payload); return true; }
